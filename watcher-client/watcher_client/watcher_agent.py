    ...
    def check(self, ):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    'Error parsing /proc/stat data. Traceback: %s', traceback.format_exc(exc))
            ...
            try:
                if not self.prev_check.get('csw') or not self.prev_check.get('intr'):
                    ...
                    delta_csw = str(self.current_check[
                                    'csw'] - self.prev_check['csw'])
                    delta_intr = str(self.current_check[
                                     'intr'] - self.prev_check['intr'])
                    self.prev_check['csw'] = self.current_check['csw']
                    self.prev_check['intr'] = self.current_check['intr']
                ...
                logger.error(
                    'Error trying to count delta of cpu interrupts and csw. Traceback: %s', traceback.format_exc(exc))
                result.extend([''] * 2)
            ...
            try:
                ...
                logger.error(
                    'Error trying to count delta of cpu usage metrics. Traceback: %s', traceback.format_exc(exc))
                result.extend([''] * 7)
        ...
        try:
            ...
            logger.error(
                'Error trying to count numprocs. Traceback: %s', traceback.format_exc(exc))
            result.append([''])
        else:
        ...
        try:
            ...
            logger.error(
                'Error opening /proc/loadavg to get numthreads. Traceback: %s', traceback.format_exc(exc))
            result.append([''])
        else:
        ...
        for mount in mounts:
            if mount.startswith("/dev"):
                ...
                try:
                    ...
                    logger.info(
                        "Failed to get block device name via /sys/devices/: %s", traceback.format_exc(exc))
...
class Mem(AbstractMetric):
    ...
    def __init__(self):
        ...
        self.vars = ('MemUsed', 'Buffers', 'Cached',
                     'MemFree', 'Dirty', 'MemTotal')
    ...
    def check(self):
        ...
        try:
            ...
            output = subprocess.Popen(
                'cat /proc/meminfo', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except Exception, e:
            ...
            if err:
                ...
                for l in info.splitlines():
                    ...
                    if name in self.vars:
                        ...
                        data.update(
                            {name: long(raw_value.split()[0]) / 1024.0})
                        #                print data
                data['MemUsed'] = data['MemTotal'] - \
                    data['MemFree'] - data['Buffers'] - data['Cached']
            result = [data['MemTotal'], data['MemUsed'], data[
                'MemFree'], 0, data['Buffers'], data['Cached']]
    ...
    def check(self, ):
        ...
        try:
            stat = subprocess.Popen(
                [cmnd], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        except Exception, exc:
...
class AgentWorker(Thread):
    ...
    def __init__(self):
        ...
        self.c_interval = 1
        self.c_file = '/tmp/watcher-default'
        self.tails = None
        self.calls = None
        ...
        while not self.finished:
            ...
            try:
                ...
                if e.errno == 32:
                    ...
                    logger.error(
                        'IOError while converting line: %s: %s', line, e)
            except Exception, e:
...
class AgentConfig:
    ...
    def parse_config(self, cfg_file='agent.cfg'):
        ...
        if config.has_section('main'):
            ...
            if config.has_option('main', 'out-file'):
                self.c_file = config.get('main', 'out-file')
...
def watcher_agent():
    fname = "/var/log/watcher-client/watcher-agent.log"
    level = logging.DEBUG
    ...
    agent_config = AgentConfig('/etc/watcher-client/watcher-agent.cfg')
